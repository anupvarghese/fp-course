...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser john fred =
    P (\i -> case parse fred i of
              UnexpectedEof -> UnexpectedEof
              ExpectedEof j -> ExpectedEof j
              UnexpectedChar c -> UnexpectedChar c
              UnexpectedString s -> UnexpectedString s
              Result j a -> parse (john a) j)

-- bindParser john fred = P (\i -> onResult (parse p i)
--                                           (\r a -> parse (john a) r))
-- | Return a parser that puts its input into the given parser and
--
...
(|||) ::
  ...
  -> Parser a
  -> Parser a
(|||) p1 p2 = P (\i -> let r = parse p1 i
                       in bool r (parse p2 i) (isErrorResult r))
...
list1 ::
  Parser a
  -> Parser (List a)
-- list1 parser1 = parser1 >>= \fredparser -> list parser1 >>= \otherfredparsers -> pure ((:.) fredparser otherfredparsers)
list1 parser1 =
  do
    fredparser <- parser1
    otherfredparsers <- list parser1
    pure ((:.) fredparser otherfredparsers)
-- (1 or many) p = p (and then) (call it f) (o or many) p (and then )(call it otherf) always ((:.) f otherf)
...
satisfy ::
  ...
  -> Parser Char
satisfy =
  -- \predicate -> character >>= \c -> bool (unexpectedCharParser c) (pure c) (predicate c)
  \predicate -> character >>= lift3 bool unexpectedCharParser pure predicate
...
surnameParser =
  upper >>= \damo ->
    (thisMany 5) lower >>= \dazza ->
    (list) lower >>= \bazza ->
    pure (damo :. dazza ++ bazza)
...
instance Applicative Parser where
  ...
  (<*>) =
    \pab pa ->
      bindParser (\eff -> mapParser (\a -> eff a) pa) pab
    -- \pab pa ->
    --   pab >>= \eff ->
    --     pa >>= \gee ->
    --       valueParser (eff gee)
      -- bindParser (\eff -> bindParser (\gee -> valueParser (eff gee)) pa) pab
